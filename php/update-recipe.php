...
// Build UPDATE query - skip image if empty (preserve existing)
if (!empty($image)) {
    $sql = "UPDATE `Recipes` SET
        ...
        `preparation_time` = ?, `cooking_time` = ?, `temperature` = ?, `serves` = ?, `makes` = ?,
        `source` = ?, `source_link` = ?, `image` = ?
        WHERE `index` = ?";
    ...
    $stmt->bind_param(
        ...
        $prepTime, $cookTime, $temperature, $serves, $makes,
        $source, $sourceLink, $image, $id
    );
} else {
    $sql = "UPDATE `Recipes` SET
        `name` = ?, `ingredients` = ?, `description` = ?, `type` = ?,
